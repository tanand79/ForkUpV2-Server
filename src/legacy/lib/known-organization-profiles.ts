/**
 * Curated website → organization profiles (Lovable parity).
 *
 * Lovable's Find Organization does NOT invent Headstrong from raw AI — it
 * resolves known domains from a coherent registry so name, location, mission,
 * and social links always belong to the SAME organization.
 *
 * Used by generate-organization-draft before calling Bedrock/Lovable AI.
 * Additive only — unknown domains still fall through to AI draft.
 */
...
/** Normalize a URL or host to a bare domain key (no www). */
export function normalizeProfileDomain(raw: string): string {
...
/**
 * Known nonprofit/business website profiles used in Lovable Sprint 1 demos.
 * Keys are bare domains (lowercase, no www).
 */
const KNOWN_BY_DOMAIN: Record<string, Omit<KnownOrganizationProfile, "website">> = {
  "headstrong.org": {
    organizationName: "Headstrong Foundation",
    missionStatement:
      "Nonprofit supporting families facing cancer, inspired by lacrosse.",
    about:
      "Headstrong Foundation provides financial, residential, and emotional support to families affected by cancer.",
    contactEmail: "",
    phone: "",
  ...
  "ymcagbw.org": {
    ...
    missionStatement:
      "Community organization supporting youth development, healthy living, and social responsibility.",
    about:
      "Community YMCA serving families across the Brandywine region.",
    contactEmail: "",
    phone: "",
  ...
  "ymcade.org": {
    ...
    missionStatement:
      "Regional YMCA organization supporting youth, families, and community wellness.",
  ...
  "ymcawestchester.org": {
    ...
    missionStatement:
      "Neighborhood YMCA offering youth programs and community services.",
  ...
  "wcyouthlacrosse.org": {
    ...
    missionStatement:
      "Youth lacrosse club supporting players, families, and local programs.",
  ...
  "greenmorerescue.org": {
    ...
    missionStatement:
      "Animal rescue organization helping farm animals and connecting the community with animal welfare.",
    about:
      "Animal rescue organization helping farm animals and connecting the community with animal welfare.",
    contactEmail: "",
    phone: "",
  ...
  "wclacrosseboosters.org": {
    ...
    missionStatement:
      "Parent-run booster club raising funds for equipment and travel.",
  ...
  "feedingamerica.org": {
    ...
    missionStatement:
      "Nationwide network of food banks working to end hunger in the United States through food rescue, meal programs, and community partnerships.",
    about:
      "Feeding America is the largest hunger-relief organization in the United States, supporting a nationwide network of food banks and meal programs so families and kids can access nutritious food.",
    contactEmail: "",
    phone: "",
    location: "Chicago, IL",
  ...
  "redcross.org": {
    ...
    about:
      "The American Red Cross provides disaster relief, blood donation services, health and safety training, and support for military families across the United States.",
    contactEmail: "",
    phone: "",
...
/**
 * Look up a curated profile by website URL or domain.
 * Exact domain match first; then known host fragments (e.g. headstrong.org).
 */
export function findKnownOrganizationProfile(
  websiteOrDomain: string,
): KnownOrganizationProfile | null {
...
/** Words too common to identify an organization alone (prevents Red Cross → Feeding America). */
 ...
 * Never match on a single weak shared token (e.g. "america").
 */
export function findKnownOrganizationByName(nameQuery: string): KnownOrganizationProfile | null {
  ...
  // Compact domain key equality only (feedingamerica === feedingamerica.org host)
  ...
  // Full name containment either direction (whole phrase, not a weak token)
  for (const [domain, profile] of Object.entries(KNOWN_BY_DOMAIN)) {
  ...
  // Strong multi-token overlap: require at least 2 distinctive tokens, or 1 long distinctive token
