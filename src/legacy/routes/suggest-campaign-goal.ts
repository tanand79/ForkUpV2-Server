...
export const suggestCampaignGoalRouter = Router();
...
/**
 * Lightweight AI goal suggestion for the Build goal screen (before methods / full draft).
 ...
 */
suggestCampaignGoalRouter.post("/suggest-campaign-goal", async (req, res) => {
  try {
    ...
    let draft: { suggestedGoal?: number | string } = {};
    try {
    ...
    const message =
      error instanceof Error ? error.message : "Could not suggest a fundraising goal.";
