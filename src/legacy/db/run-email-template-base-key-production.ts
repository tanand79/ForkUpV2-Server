/**
 * Run email_templates.base_template_key migration against production RDS.
 * Usage: npm run db:email-template-base-key:production
 *
 * Additive only: ADD COLUMN IF NOT EXISTS base_template_key + backfill + index.
...
async function main() {
  ...
  const { migrateEmailTemplateBaseKey } = await import(
    "./migrate-email-template-base-key.js"
  );
