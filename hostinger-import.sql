-- ForkUp V1 — import this file in Hostinger phpMyAdmin
-- Database: u847374826_ForkUp
-- hPanel → Databases → Enter phpMyAdmin → Import → choose this file
...
CREATE TABLE IF NOT EXISTS campaigns (
  ...
  campaign_status ENUM(
    'draft', 'invitation_phase', 'ready_to_launch', 'live', 'closed', 'settlement'
...
CREATE TABLE IF NOT EXISTS campaign_methods (
  ...
  method_type ENUM(
    'dine_and_donate', 'shop_and_donate', 'service_giveback',
    'virtual_donations', 'ambassador_fundraising', 'guest_bartending_event'
  ) NOT NULL,
  method_name VARCHAR(255) NOT NULL,
  method_start_date DATE NULL,
  method_end_date DATE NULL,
  method_status ENUM(
    'draft', 'invited', 'pending_acceptance', 'accepted',
...
CREATE TABLE IF NOT EXISTS campaign_business_locations (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT fk_cbl_campaign FOREIGN KEY (campaign_id) REFERENCES campaigns(id) ON DELETE CASCADE,
  CONSTRAINT fk_cbl_method FOREIGN KEY (method_id) REFERENCES campaign_methods(id) ON DELETE CASCADE,
  CONSTRAINT fk_cbl_business FOREIGN KEY (business_id) REFERENCES businesses(id) ON DELETE RESTRICT,
  CONSTRAINT fk_cbl_location FOREIGN KEY (location_id) REFERENCES business_locations(id) ON DELETE RESTRICT
);
...
CREATE TABLE IF NOT EXISTS participation_intents (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_intents_campaign FOREIGN KEY (campaign_id) REFERENCES campaigns(id) ON DELETE CASCADE,
  CONSTRAINT fk_intents_supporter FOREIGN KEY (supporter_id) REFERENCES supporters(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS campaign_participants (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_participants_campaign FOREIGN KEY (campaign_id) REFERENCES campaigns(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS receipts (
  ...
  CONSTRAINT fk_receipts_campaign FOREIGN KEY (campaign_id) REFERENCES campaigns(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS settlements (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_settlements_campaign FOREIGN KEY (campaign_id) REFERENCES campaigns(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS success_engine_actions (
  ...
  campaign_id INT NOT NULL,
  action_type ENUM(
    'launch_email', 'one_week_reminder', 'mid_campaign_reminder', 'final_push_reminder',
    'results_email', 'ambassador_recruitment', 'guest_bartender_recruitment',
    'business_promotion', 'receipt_reminder'
  ) NOT NULL,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_success_actions_campaign FOREIGN KEY (campaign_id) REFERENCES campaigns(id) ON DELETE CASCADE
);
