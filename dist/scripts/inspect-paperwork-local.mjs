...
const cols = await pool.query(`SELECT column_name, data_type, character_maximum_length, is_nullable, column_default
...
const t = await pool.query(`SELECT column_name, data_type, character_maximum_length, is_nullable, column_default
   FROM information_schema.columns
   WHERE table_schema='public' AND table_name='business_paperwork_tokens'
   ORDER BY ordinal_position`);
console.log("tokens:", JSON.stringify(t.rows, null, 2));
const cons = await pool.query(`SELECT conname, pg_get_constraintdef(oid) AS def
   FROM pg_constraint
   WHERE conrelid = 'business_paperwork_tokens'::regclass`);
console.log("constraints:", JSON.stringify(cons.rows, null, 2));
const idx = await pool.query(`SELECT indexdef FROM pg_indexes WHERE tablename='business_paperwork_tokens'`);
console.log("indexes:", JSON.stringify(idx.rows, null, 2));
const idCol = await pool.query(`SELECT column_default, is_identity, identity_generation
   FROM information_schema.columns
   WHERE table_name='business_paperwork_tokens' AND column_name='id'`);
console.log("id meta:", JSON.stringify(idCol.rows, null, 2));
const chk = await pool.query(`SELECT conname, pg_get_constraintdef(oid) AS def
   FROM pg_constraint
...
await pool.end();
//# sourceMappingURL=inspect-paperwork-local.mjs.map