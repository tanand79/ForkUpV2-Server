...
async function schema(label, url) {
...
if (!rdsUrl)
    throw new Error("missing DATABASE_URL_PRODUCTION");
if (!/\/forkupv2(\?|$)/.test(rdsUrl)) {
    rdsUrl = rdsUrl.replace(/\/([^/?]+)(\?|$)/, "/forkupv2$2");
}
console.log("LOCAL", (localUrl.match(/@([^/?]+)/) || [])[1]);
console.log("RDS", (rdsUrl.match(/@([^/?]+)/) || [])[1], "db=", (rdsUrl.match(/\/([^/?]+)(\?|$)/) || [])[1]);
...
//# sourceMappingURL=compare-local-rds-schema.mjs.map