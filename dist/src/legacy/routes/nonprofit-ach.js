...
exports.nonprofitAchRouter = void 0;
...
exports.nonprofitAchRouter = (0, express_1.Router)();
const SIG_DIR = path_1.default.join(process.cwd(), "uploads", "nonprofit-ach-signatures");
...
async function userMayAccessNonprofit(userId, userEmail, nonprofitId) {
    const { rows } = await pool_1.pool.query(`SELECT id, contact_email FROM nonprofits WHERE id = $1`, [nonprofitId]);
    if (rows.length === 0) {
    ...
    const { rows: membership } = await pool_1.pool.query(`SELECT 1 FROM organization_users
     WHERE organization_type = 'nonprofit' AND organization_id = $1 AND user_id = $2`, [nonprofitId, userId]);
    if (membership.length === 0) {
        return { ok: false, status: 403, error: "Not authorized for this nonprofit" };
    }
    return { ok: true };
}
function saveSignatureBase64(nonprofitId, base64) {
    ensureSigDir();
    let raw = base64;
    if (raw.includes(","))
        raw = raw.split(",")[1] ?? "";
...
exports.nonprofitAchRouter.get("/nonprofits/:nonprofitId/ach", async (req, res) => {
    try {
        ...
        const { rows } = await pool_1.pool.query(`SELECT id, organization_name,
...
exports.nonprofitAchRouter.post("/nonprofits/:nonprofitId/ach", async (req, res) => {
    try {
        const user = await (0, auth_1.resolveAuthUser)((0, auth_1.bearerToken)(req));
        if (!user) {
            res.status(401).json({ error: "Authentication required" });
...
//# sourceMappingURL=nonprofit-ach.js.map