...
exports.extractReceiptWithBedrock = extractReceiptWithBedrock;
const ai_chat_1 = require("./ai-chat");
const image_mime_sniff_1 = require("./image-mime-sniff");
...
function resultFromBedrockExtract(parsed, claimedSubtotal, rawJson) {
    ...
    const subtotal = (0, mindee_ocr_1.normalizeSubtotal)(rawSubtotal, total, tax, totalLineItems, tip);
    const claimed = claimedSubtotal != null && claimedSubtotal > 0 ? roundMoney(claimedSubtotal) : 0;
...
async function extractReceiptWithBedrock(input) {
    if ((0, ai_chat_1.aiProviderName)() !== "bedrock") {
        return manualOnly("AI receipt OCR requires AWS Bedrock credentials.", input.claimedSubtotal);
    }
    try {
        ...
        const mime = (0, image_mime_sniff_1.normalizeImageMediaType)(buffer, input.mimeType || "image/jpeg");
        const modelId = await (0, ai_chat_1.resolveReceiptBedrockModelId)(input.bedrockModelId, input.userId);
        const text = await (0, ai_chat_1.aiChatWithImages)({
            system: RECEIPT_VISION_SYSTEM,
            user: "Extract all visible receipt, invoice, or check fields from this image for donation eligibility review.",
            json: true,
            maxTokens: 1024,
            temperature: 0.1,
        ...
        const parsed = (0, ai_chat_1.parseAiJson)(text);
        const hasSignal = readText(parsed.merchantName) ||
            readNumber(parsed.total) > 0 ||
            readNumber(parsed.subtotal) > 0 ||
            readText(parsed.receiptNumber);
        if (!hasSignal) {
            return manualOnly("AI could not read this receipt. Enter subtotal manually.", input.claimedSubtotal);
        }
        return resultFromBedrockExtract(parsed, input.claimedSubtotal, parsed);
    }
    catch (err) {
        console.error("Bedrock receipt OCR error:", err);
        return manualOnly("AI could not read the receipt. Enter subtotal manually.", input.claimedSubtotal);
    }
}
//# sourceMappingURL=bedrock-receipt-ocr.js.map